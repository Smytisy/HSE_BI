...
1. Clone the repository:
   ```sh
   git clone https://github.com/Smytisy/HSE_BI.git
   cd HSE_BI/TechProgramming/TZ2
...
### Что реализовано  
Все что просят по критериям оценки и все что было в чате в тг.  
Функция чтения из файла, функции min max sum multi  
тесты проверки коректности, скорости работы, свой тест в виде крайних значений, тесты на ошибки, тесты в парадигме given-when-then  
отображние с графика зависимости времени от количесвта элементов c помощью JFreeChart
Ну а также CI/CD и telegram оповещение, бейджик
   
#### Вот так выглядит график зависимости времени выполнения от количества рандомно сгенерирорванных элементов 
На каждом этапе интерации количество элементов увеличивается в 2 раза, генерируя новый массив. Еще мы можем видеть значительно большее время в периоде на интервале от 200к до 600к элементов, т к скорее всего сгенерированные элементы на данных количествах оказались больше, чем на генерации в около 1кк. 
